...
test.describe('Wishlist Test Suite as a user', () => {
  ...
  test('should allow user to add multiple products to the wishlist', async () => {
    ...
    const countOfProducts =
      await wishlistPage.getProductsCountFromTheSmallIcon();
  ...
  test('should prompt login when trying to add to wishlist without authentication', async () => {
    ...
    await expect(loginPrompt).toContain(
      textData.wishlistPage.loginPromptMessagePart1
    );
    await expect(loginPrompt).toContain(
      textData.wishlistPage.loginPromptMessagePart2
    );
    await expect(loginPrompt).toContain(
      textData.wishlistPage.loginPromptMessagePart3
    );
  });
});
