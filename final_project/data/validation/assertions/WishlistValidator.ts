...
export class WishlistValidations {
    ...
    async assertDuplicateErrorMessage(errorMessageLocator: Locator): Promise<void> {
        await expect(errorMessageLocator).toBeVisible();
    }

    async assertPromptToLogInAppears(expectedKeywords: string[]): Promise<void> {
        const loginPrompt = await this.page.locator(wishlistPage.loginPromptMessage).textContent();
    ...
    async assertEmptyWishlistMessage(expectedMessage: string): Promise<void> {
        const emptyMessage = await this.page.locator(wishlistPage.wishlistEmptyMessage).textContent();
