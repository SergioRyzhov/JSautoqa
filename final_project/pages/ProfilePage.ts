...
export class ProfilePage extends Base {
    ...
    async assertProfileDetails() {
        ...
        for (const key in textData.profilePage.fieldName) {
            ...
            await expect(this.page.locator(profilePage.field(textData.profilePage.fieldName[profileKey]))).toBeVisible();
        }
    }

    async assertEditFirstName(newName: string) {
        await this.navigateToPage(pageEndpoints.profileEditPage);
        await this.page.fill(profilePage.inputField(textData.profilePage.fieldName.firstName), newName);
    ...
    async assertFirstNameChange(newName: string) {
        const newNameText = await this.page.locator(profilePage.userNameHeader).textContent();
