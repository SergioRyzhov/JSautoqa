...
export class WishlistPage extends Base {
    ...
    async assertProductsInTheWishlist(): Promise<boolean> {
        ...
        const sectionCount = await this.page.locator(wishlistPage.productsCountInTheWishlist).count();
    ...
    async getProductsCountFromTheSmallIcon(): Promise<number> {
        ...
        const countOfProducts = await this.page.locator(wishlistPage.smallIconCount).textContent();
    ...
    async removeProductsFromWishlist() {
        await this.navigateToPage(pageEndpoints.wishlistPage);
        let removeButtons = await this.page.locator(wishlistPage.removeButtonsOfProductsInTheWishlist);
        ...
        for (let i = 0; i < initialButtonsCount; i++) {
            ...
            await waitUntilCountChange(this.page, wishlistPage.removeButtonsOfProductsInTheWishlist, beforeRemoveCount);
            removeButtons = await this.page.locator(wishlistPage.removeButtonsOfProductsInTheWishlist);
        }
    }

    async addProductsToWishlistFast(n: number) {
        await this.navigateToPage(pageEndpoints.productsPage);
        const addToWishlistButtons = this.page.locator(wishlistPage.addToWishlistButtons);
        ...
        if (n <= buttonsCount) {
            for (let i = 0; i < n; i++) {
                ...
                const oldWishQty = await this.page.locator(wishlistPage.wishQuantitySmallIcon).textContent();
                ...
                if (ariaPressed === 'false') {
                    ...
                    await this.page.waitForFunction(
                        ...
                        { oldQty: oldWishQtyCleaned, wishlistSelector: wishlistPage.wishQuantitySmallIcon }
                    );
                }
            }
        }
    ...
    async getEmptyWishlistMessage(): Promise<string | null> {
        await this.page.waitForSelector(wishlistPage.wishlistEmptyMessage, { state: 'visible' });
        return this.page.locator(wishlistPage.wishlistEmptyMessage).textContent();
    }

    async addChosenProductToWishlist() {
        await waitForPageLoadAndElVisible(this.page, wishlistPage.addChosenProductToWishlist);
